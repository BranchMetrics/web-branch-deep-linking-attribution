...
elements.smartBannerStyles = function() {
  ...
  a.type = "text/css";
  a.id = "branch-css";
  a.innerHTML = bannerCSS;
  a.innerHTML = mobileUserAgent ? a.innerHTML + bannerMobileCSS : a.innerHTML + bannerDesktopCSS;
  return a;
};
elements.appendSmartBannerActions = function(a) {
  var b = document.createElement("div");
  if (mobileUserAgent) {
    branch.link({channel:"appBanner", data:a.data || {}}, function(a, b) {
      document.getElementById("branch-mobile-action").href = b;
    }), b.innerHTML = bannerMobileActionHTML;
  } else {
    b.innerHTML = bannerDesktopActionHTML;
    var d = addEventListener("click", function() {
      sendBannerSMS(a);
    });
  }
  document.getElementById("branch-banner-action").appendChild(b);
  (document.getElementById("branch-sms-send") || {onclick:""}).onclick = d;
...
}, sendBannerSMS = function(a) {
  var b = document.getElementById("branch-sms-phone");
  if (b) {
    var d = b.value;
    validatePhoneNumber(d) ? branch.SMSLink({phone:d, data:a.data || {}}, function() {
      document.getElementById("branch-sms-block").innerHTML = bannerLinkSentHTML(d);
    }) : b.className = "error";
  }
}, closeBranchBanner = function() {
  setTimeout(function() {
...
}, mobileUserAgent = navigator.userAgent.match(/android|i(os|p(hone|od|ad))/i), validatePhoneNumber = function(a) {
  return/^\d{7,}$/.test(a.replace(/[\s()+\-\.]|ext/gi, ""));
}, bannerHTML = function(a) {
...
}, bannerDesktopActionHTML = '<div id="branch-sms-block"><input type="phone" name="branch-sms-phone" id="branch-sms-phone" placeholder="(999) 999-9999"><button id="branch-sms-send">Send Link</button></div>', bannerLinkSentHTML = function(a) {
  return'<span class="sms-sent">App link sent to ' + a + "!</span>";
}, bannerMobileActionHTML = '<a id="branch-mobile-action" href="#">View in App</a>', bannerCSS = "body { -webkit-transition: all " + 1.5 * animationSpeed / 1E3 + "s ease; transition: all 0" + 1.5 * animationSpeed / 1E3 + "s ease; }#branch-banner { top: -76px; width: 100%; font-family: Helvetica Neue, Sans-serif; -webkit-font-smoothing: antialiased; -webkit-text-size-adjust: none; -webkit-tap-highlight-color: rgba(0,0,0,0); -webkit-user-select: none; -moz-user-select: none; user-select: none; -webkit-transition: all " + 
animationSpeed / 1E3 + "s ease; transition: all 0" + animationSpeed / 1E3 + 's ease; }#branch-banner .close-x { float: left; font-weight: 400; color: #aaa; font-size: 20px; margin-top: 13px; margin-right: 6px; margin-left: 0; cursor: pointer; }#branch-banner .content { position: absolute; width: 100%; height: 76px; z-index: 99999; background: rgba(255, 255, 255, 0.95); color: #333; border-bottom: 1px solid #ddd; }#branch-banner .content .left { width: 70%; float: left; padding: 8px 8px 8px 8px; }#branch-banner .content .left .icon img { width: 60px; height: 60px; margin-right: 6px; }#branch-banner .content .right a { font-size: 16px; font-weight: 500; color: #007aff; }#branch-banner-action div { float: right; margin-right: 8px; }#branch-banner .content:after { content: ""; position: absolute; left: 0; right: 0; top: 100%; height: 1px; background: rgba(0, 0, 0, 0.2); }#branch-banner .content .left .details { margin-top: 3px; padding-left: 4px; overflow:hidden; }#branch-banner .content .left .details .title { font: 14px/1.5em HelveticaNeue-Medium, Helvetica Neue Medium, Helvetica Neue, Sans-serif; color: rgba(0, 0, 0, 0.9); display: inline-block; }#branch-banner .content .left .details .description { font-size: 12px; font-weight: normal; line-height: 1.5em; color: rgba(0, 0, 0, 0.5); display: block; }#branch-banner .content .right { display:inline-block; margin-top: 25px; }', 
bannerMobileCSS = "#branch-banner .content .left { width: 70% }#branch-banner .content .right { width: 30% }#branch-banner { position: absolute; }", bannerDesktopCSS = "#branch-banner .content .left { width: 50% }#branch-banner .content .right { width: 50% }#branch-banner { position: fixed; }#branch-banner .content .right input { font-weight: 400; border-radius: 4px; height: 30px; border: 1px solid #ccc; padding: 5px 7px 4px; width: 125px; font-size: 14px; }#branch-banner .content .right button { margin-top: 0px; display: inline-block; height: 30px;; float: right; margin-left: 5px; font-weight: 400; border-radius: 4px; border: 1px solid #ccc; background: #fff; color: #000; padding: 0px 12px; }#branch-banner .content .right button:hover { border: 1px solid #BABABA; background: #E0E0E0; }#branch-banner .content .right input:focus, button:focus { outline: none; }#branch-banner .content .right input .error { color: red; border-color: red; }#branch-banner .content .right span { display: inline-block; font-weight: 100; margin: 7px 9px; font-size: 12px; }";
// Input 3
...
Branch.prototype.banner = function(a) {
  document.getElementById("branch-banner") || (document.head.appendChild(elements.smartBannerStyles()), document.body.appendChild(elements.smartBannerMarkup(a)), document.getElementById("branch-banner").style.top = "-76px", elements.appendSmartBannerActions(a), elements.triggerBannerAnimation());
};
// Input 6
