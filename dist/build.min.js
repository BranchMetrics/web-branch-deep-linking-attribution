...
d.D)),null);if(0===d.A||!d.A)return g(Error(A("Branch SDK not initialized")),null)}m.unshift(g);b.apply(d,m)})}}function Z(){if(!(this instanceof Z))return kd||(kd=new Z),kd;this.ia=ec();this.g=new Fb;this.$=new Lb;this.u=[];this.ca="web2.84.0";this.l={};this.B=this.A=0;this.D=null}
...
function(f){return a(f||null)})});Z.prototype.link=X(2,function(a,b){var c=Ja(b),d=this.o;this.j(wb,c,function(e,f){if(e)return a(e,Ca(d,c));a(null,f&&f.url)})});
Z.prototype.qrCode=X(2,function(a,b,c){Ja(b).qr_code_settings=w(kb(c||{}));this.j(xb,Ja(b),function(d,e){function f(){}d||(f.rawBuffer=e,f.base64=function(){if(this.rawBuffer){const m=Array.from(new Uint8Array(this.Ha)).map(k=>String.fromCharCode(k)).join("");return btoa(m)}throw Error("QrCode.rawBuffer is empty.");});return a(d||null,f||null)})});
