...
function Kb(a,b,c,d,e,f,m,l){var g=Date.now(),n=oa,k=window.XMLHttpRequest?new XMLHttpRequest:new ActiveXObject("Microsoft.XMLHTTP");l&&(k.responseType=l);k.ontimeout=function(){G(w,n,pa(g));f(Error("Request timed out"),null,504)};k.onerror=function(q){f(Error(q.error||"Error in API: "+k.status),null,k.status)};k.onreadystatechange=function(){if(4===k.readyState)if(G(w,n,pa(g)),200===k.status){if("arraybuffer"===k.responseType)var q=k.response;else if(m)q=k.responseText;else try{q=t(k.responseText)}catch(u){q=
...
if(x)for(var l=["browser_fingerprint_id","alternative_browser_fingerprint_id","identity_id","session_id"],g=0;g<l.length;g++)c.hasOwnProperty(l[g])&&delete c[l[g]];l=Ib(a,b,c);if(l.error)return e(Error(v({message:l.error,endpoint:b.endpoint,data:c})));var n="";if("GET"===b.method)var k=l.url+"?"+l.data;else k=l.url,n=l.data;var q;d.get("use_jsonp")||b.R?q=c:q=n;var u=ja;if(x&&ta(k,c))return sa?m(Error("Requested operation cannot be completed since tracking is disabled"),null,300):m(null,{},200);var R=
...
""))?(P(b,"willSendBannerSMS"),g.setAttribute("disabled",""),l.setAttribute("disabled",""),g.style.opacity=".4",l.style.opacity=".4",n.style.opacity="1",l.className="",b.sendSMS(u,d,c,function(R){R?(P(b,"sendBannerSMSError"),e()):(P(b,"didSendBannerSMS"),f(),setTimeout(function(){k.removeChild(q);m()},3E3))})):e()}}
...
var k=b.K?f.contentWindow.document:document;if(D()){if(b.open_app=b.ta,b.append_deeplink_path=b.ja,b.make_new_link=b.qa,b.deepview_type="banner",a.deepview(c,b),g=k.getElementById("branch-mobile-action"))g.onclick=function(u){u.preventDefault();a.deepviewCta()}}else k.getElementById("sms-form")?k.getElementById("sms-form").addEventListener("submit",function(u){u.preventDefault();Zb(k,a,b,c)}):f.onload=function(){k=f.contentWindow.document;k.getElementById("sms-form")&&k.getElementById("sms-form").addEventListener("submit",
function(u){u.preventDefault();Zb(k,a,b,c)})};g=Pb("margin-top");var q=Pb("margin-bottom");Mb(document.body,"branch-banner-is-active");"top"===b.position?document.body.style.marginTop=Qb(g):"bottom"===b.position&&(document.body.style.marginBottom=Qb(q));if(g=k.getElementById("branch-banner-close"))g.onclick=function(u){u.preventDefault();P(a,"willCloseBanner");e({},function(){P(a,"didCloseBanner")})};if(g=k.getElementById("branch-banner-modal-background"))g.onclick=function(u){u.preventDefault();
...
u[3]&&0===parseFloat(u[3])&&(l.style.boxShadow="none")}catch(R){}q=d;l.contentWindow.document.getElementById("branch-mobile-action").innerHTML=q;P(U,"willShowJourney",V);Mc(l,m);c(l)};document.body.appendChild(l)}
...
B.branch_view_id||K.event_data.branch_view_data.id,K.event_data.branch_view_data,E,K.journey_link_data):((K.auto_branchify||!n&&Ka("branchify_url")&&X(g))&&this.branch.deepview({},{make_new_link:!1,open_app:!0,auto_branchify:!0}),P(U,"willNotShowJourney")));x&&(sa=!0)})})}function f(p){q&&(p.identity=g.m);return p}function m(p){var r={sdk:"2.62.0",branch_key:g.o},B=O(g.h)||{},E=O(g.h,!0)||{};E.browser_fingerprint_id&&(r._t=E.browser_fingerprint_id);Ia()||Ja()||g.i(jb,r,function(K,Ha){K&&(g.u=3,g.B=
