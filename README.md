...
branch.banner({
    ...
    openAppButtonText: 'Open',              // (default 'View in app') Text to show on button if the user has the app installed
    downloadAppButtonText: 'Download',      // (default 'Download App') Text to show on button if the user does not have the app installed
    sendLinkText: 'Send Link',              // (default 'Send Link') Text to show on desktop button to allow users to text themselves the app
    phonePreviewText: '+44 9999-9999',      // The default phone placeholder is a US format number, localize the placeholder number with a custom placeholder with this option
    showiOS: true,                          // (default `true`) Show banner on iOS devices (both iPhones and iPads)?
    showiPad: true,                         // (default `true`) Show banner on iPads (this overrides showiOS)?
    showAndroid: true,                      // (default `true`) Show banner on Android devices?
    showBlackberry: true,                   // (default `true`) Show banner on Blackberry devices?
    showWindowsPhone: true,                 // (default `true`) Show banner on Windows Phone devices?
    showKindle: true,                       // (default `true`) Show banner on Kindle devices?
    showDesktop: true,                      // (default `true`) Show banner on desktop devices?
    iframe: true,                           // (default `true`) Show banner in an iframe, recommended to isolate Branch banner CSS
    disableHide: false,                     // Should the user have the ability to hide the banner? (show's X on left side)
    forgetHide: false,                      // Should we show the banner after the user closes it? Can be set to true, or an integer to show again after X days
    respectDNT: false,                      // (default `false`) Should we skip showing the banner when a user's settings show 'Do Not Track'?
    mobileSticky: false,                    // (default `false`) Determines whether the mobile banner will be set `position: fixed;` (sticky) or `position: absolute;`, defaults to false *this property only applies when the banner position is 'top'
    desktopSticky: true,                    // (default `true`) Determines whether the desktop banner will be set `position: fixed;` (sticky) or `position: absolute;`, defaults to true *this property only applies when the banner position is 'top'
