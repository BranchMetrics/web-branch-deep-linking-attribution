...
describe('Branch', function () {
  ...
  function initBranch(runInit, keepStorage) {
    ...
    sandbox.stub(
      branch._server,
      'request',
      function (resource, obj, storage, callback) {
        requests.push({
          resource: resource,
          obj: obj,
          callback: callback,
  ...
  describe('init', function () {
    ...
    it('should call has_app if session present but no link_identifier from get param', function (done) {
      var branch = initBranch(false);
      var assert = testUtils.plan(3, done);
      branch.init(branch_sample_key);
      requests[0].callback(null, browser_fingerprint_id);
    ...
    it('should not call _r if session present but no link_identifier and safari 11 or greater', function (done) {
      var safari11Ua =
        'Mozilla/5.0 (iPod touch; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.28 (KHTML, like Gecko) Version/11.0 Mobile/15A5318g Safari/604.1';
      setUserAgent(safari11Ua);
      if (navigator.userAgent !== safari11Ua) {
  ...
  describe('setIdentity', function () {
    ...
    it('should invoke callback with error when a null value for identity is passed', function (done) {
      ...
      branch.setIdentity(null, function (err, res) {
        assert.strictEqual(
          err,
          utils.messages.missingIdentity,
  ...
  describe('track', function () {
    ...
    it('should print console warning about method deprecation for track', function () {
      ...
      branch.track();
      assert(
        spy.calledWith('track method currently supports only pageview event.'),
      );
    });
  });
  ...
  describe('banner', function () {
    ...
    sandbox.stub(
      ...
      function (server, branchViewData, data) {
        return 'desktop';
      },
    );
  ...
  describe('deepview', function () {
    ...
    beforeEach(function () {
      sinon.stub(utils, 'getWindowLocation').returns(windowLocation);

      sinon.stub(utils, 'getHostedDeepLinkData').returns({});

      // check starting with 10th call since first 9 are called by openGraphDataAsObject
      sinon
        .stub(utils, 'getOpenGraphContent')
        .onCall(10)
        .returns(ogTitle)
        .onCall(11)
        .returns(ogDescription)
        .onCall(12)
        .returns(ogImage)
        .onCall(13)
        .returns(ogVideo)
