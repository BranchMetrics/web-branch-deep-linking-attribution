...
Branch.prototype['init'] = wrap(callback_params.CALLBACK_ERR_DATA, function(done, branch_key, options) {
	...
	if (sessionData  && sessionData['session_id']) {
		...
		if (CORDOVA_BUILD) { // jshint undef:false
			...
			if (web_session.readKeyValue('identity_id', self._storage)) {
				exec(function(data) {
					data['identity_id'] = web_session.readKeyValue('identity_id', self._storage);
					data['device_fingerprint_id'] = web_session.readKeyValue('device_fingerprint_id', self._storage);
