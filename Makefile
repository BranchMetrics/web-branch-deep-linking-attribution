...
ONPAGE_TEST=$(subst ",\",$(shell perl -pe 'BEGIN{$$sub="../dist/build.min.js"};s\#SCRIPT_URL_HERE\#$$sub\#' src/onpage.js | $(COMPILER) | node transform.js branch_sdk))

.PHONY: clean

all: dist/build.min.js dist/build.js README.md TITANIUM_GUIDE.md WEB_GUIDE.md example.html test/branch-deps.js dist/build.min.js.gz dist/build.min.js.gz test/integration-test.html
clean:
